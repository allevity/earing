                ...
                for ear = experiment.models
                    for processing = experiment.processings
                        for htk = experiment.htks
                            ...
                            if results.has_run(single)
                                fprintf('Single has run: %s\n', single.output_folder_name);
                                continue
                            end
                            ...
                            for wav_file = dataset.data.input
                                ...
                                % Need to have correct SingleRun; it remembers usr files
                                single.wav2usr(wav_path, wav_type);
                            end
                            
                             % Log on
                            diary(fullfile(single.output_folder, 'run.log'))
                            disp(datestr(now));
                            ...
                            parameters = struct(...
                                'ear', single.model.parameters, ...
                                'processing', single.processing.parameters);
                            save(fullfile(single.output_folder, 'parameters.mat'), 'parameters')
